...
export function ReleaseNotes({ isOpen, onClose }) {
    ...
    return (
        <div class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 backdrop-blur-sm animate-fade-in" onClick={onClose}>
            <div
            ...
            >
                ...
                <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-border flex justify-between items-center bg-surface-alt/50 flex-shrink-0">
                    <div class="flex items-center gap-2 sm:gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-text-muted">Version {releaseNotes[0].version}</p>
                        </div>
                    </div>
                    <button onClick={onClose} class="p-2 text-text-muted hover:text-text hover:bg-surface-alt rounded-lg transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" class="sm:w-6 sm:h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="p-4 sm:p-6 overflow-y-auto custom-scrollbar flex-1">
                    {releaseNotes.map((release, index) => (
                        <div key={release.version} class={`mb-6 sm:mb-8 last:mb-0 ${index > 0 ? 'opacity-80' : ''}`}>
                            <div class="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1 sm:gap-0 mb-2 sm:mb-3">
                                <h3 class="text-base sm:text-lg font-semibold text-text">{release.title}</h3>
                            ...
                            <ul class="space-y-2">
                                {release.changes.map((change, i) => (
                                    <li key={i} class="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-text-muted leading-relaxed">
