            ...
            <div class="bg-surface card w-full max-w-2xl max-h-[80vh] flex flex-col animate-scale-in">
                ...
                <div class="p-4 border-b border-border flex justify-between items-center">
                    <h3 class="text-lg font-bold text-text">
                        {step === "selection" ? `Delete Files: ${playlistName}` : "Confirm Deletion"}
                    </h3>
                ...
                <div class="flex-1 overflow-y-auto p-4">
                    {loading && step === "selection" ? (
                        ...
                        files.length === 0 ? (
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between mb-4">
                                    <span class="text-sm text-text-muted">{selectedFiles.size} selected</span>
                                    <button onClick={toggleAll} class="text-sm text-primary hover:underline">
                                        {selectedFiles.size === files.length ? "Deselect All" : "Select All"}
                                    </button>
                                </div>
                                {files.map(file => (
                                    <label key={file} class="flex items-start gap-3 p-2 hover:bg-surface-alt rounded cursor-pointer group">
                                        <input
                                            type="checkbox"
                                            ...
                                            class="mt-1"
                                        />
                                        <span class="text-sm text-text break-all font-mono opacity-80 group-hover:opacity-100">{file}</span>
                                    </label>
                                ))}
                            </div>
                        )
                        ...
                        <div class="text-center p-8 space-y-4">
                            ...
                            <p class="text-lg font-medium text-text">Are you absolute sure?</p>
                            <p class="text-text-muted">
                                This will permanently delete <strong>{selectedFiles.size}</strong> files from your disk.
                ...
                <div class="p-4 border-t border-border flex justify-end gap-3 bg-surface-alt/50">
                    ...
                    {step === "selection" ? (
                        ...
                        >
                            {loading && <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>}
                            Confirm Delete
                        </button>
                    )}
                </div>
